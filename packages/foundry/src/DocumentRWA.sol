...
contract DocumentRWA is ERC721, ERC721URIStorage, Ownable {
    ...
    enum FieldType { TEXT, NUMBER, DATE, ADDRESS, BOOL, BYTES }
    struct TemplateField {
        string fieldName;      // consider <= 64 chars
        FieldType fieldType;
        bool isRequired;
        string defaultValue;   // consider <= 128 chars
    }
     ...
     */
    function _isAllowedFieldType(string memory t) internal pure returns (bool) {
    bytes32 h = keccak256(bytes(t));
    return h == keccak256("text")
        || h == keccak256("number")
        || h == keccak256("date")
        || h == keccak256("address")
        || h == keccak256("bool")
    ...
    function createTemplate(
        ...
        for (uint256 i = 0; i < fieldNames.length; i++) {
            ...
            require(bytes(fieldTypes[i]).length <= 32, "Field type too long");
